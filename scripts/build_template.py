...
SCALAR_FIELD_REPLACEMENTS: list[tuple[str, str, str, int]] = [
    ...
    # Despesas
    ("Taxa de Manutenção Processual",    "{{Inserir valor}}", "{{despesas.taxa_manutencao_processual}}", 0),
]
...
def replace_despesas_value_cells(xml: str) -> str:
    ...
    targets = [
        ("Despesas Gerais", "{{despesas.gerais_descricao}}"),
        ("Despesas Específicas", "{{despesas.especificas_descricao}}"),
    ]

    for label, placeholder in targets:
        replaced = False
        for m in row_re.finditer(xml):
            ...
            if len(cells) < 2:
                continue
            if label not in _para_text(cells[0].group(0)):
                continue
            value_cell = cells[-1].group(0)
            new_value_cell = _replace_cell_with_placeholder(value_cell, placeholder)
            new_row = row[: cells[-1].start()] + new_value_cell + row[cells[-1].end():]
            xml = xml[: m.start()] + new_row + xml[m.end():]
            replaced = True
            break
        if not replaced:
            print(f"  ! linha {label!r} nao encontrada")
    return xml
...
def add_conditionals(xml: str) -> str:
    ...
    xml = wrap_range(
        xml,
        "3. HONORÁRIOS DA ATUAÇÃO CONSULTIVA",
        "4. HONORÁRIOS DE ASSESSORIA CONTENCIOSA",
    ...
    xml = wrap_range(
        xml,
        "4. HONORÁRIOS DE ASSESSORIA CONTENCIOSA",
        "5. DESPESAS APLICÁVEIS A ESTE ESCOPO",
